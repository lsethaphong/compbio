...
#my $url="http://gila.bioengr.uic.edu/cgi-bin/lips/script.cgi";
my $url="/home/rack/project/CODEWORK/uic_lips.pl";
#my $url="/usr/lib/cgi-bin/lips.cgi";
print "helix counter is $helix_counter\n";
for($helix_num=1;$helix_num<=$helix_counter;$helix_num++)
{   
    print "$helix_num of $helix_counter\n";
    my $sequences="";
    my $first_num=$helix_starts[$helix_num-1];
    my $last_num=$helix_ends[$helix_num-1];
    print "first_num is first number $first_num residue and last $last_num of $seq_count\n";
    ...
    my $suublength=length($sequence);
    my $suubstring=substr($sequence,$first_num-1,$last_num-$suublength);

    #$data=`curl -s $url -d sequence='$sequences' -d num=$first_num`;
    print "$sequence \n";

    $data=`perl $url sequence='$suubstring' num=$first_num`;
    #$data=`perl $url $sequence $first_num`;
    print DATA $data;
    print "helix count is $helix_num of $helix_counter";
    print "\n first number is $first_num";
    print "$data\n";
...
sub parse_lips
{
    ...
    my $highest_lip_index=""; # was quotes
    my $lowest_lip_index="";
    my $highest_lipo_index=1; #initialize these values
    my $lowest_lipo_index=1; #initialize these values
    ...
    print "$highest_lipo_index \n";
    print "$lowest_lipo_index \n";
